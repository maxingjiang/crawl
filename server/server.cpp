...
public:
  ...
  void deliver(const chat_message& msg)
  {
    ...
    if (!write_in_progress)
    {
	  ...
	  std::cout<<"send data:"<<write_msgs_.front().data()<<":"<<write_msgs_.front().length()<<std::endl;
/*
	  ...
	  CURL *curl;
	  vector<busPositionInfo> busPositionInfos = server_run(curl, "1", "9002");
	  boost::asio::async_write(socket_,
	      		  boost::asio::buffer("   6123456",
	              10),
	            boost::bind(&chat_session::handle_write, shared_from_this(),
	              boost::asio::placeholders::error));
    }
