...
Pool::Pool(string host,string user,string pass,string dbname,int poolSize)
{
	...
	for(int i=0; i<poolSize_; i++)
	{
		...
		if(!conn)
		{
			cout <<__FILE__<<__LINE__<< "xPool: new Connection Operation failed" << endl;
			return ;
		}
		vec[i].first = conn;
		vec[i].second = false;
...
Connection::Connection(string host,string user,string pass,string dbname)
{
	...
	{
		printf( "%s:%d, xPool: Error connecting to database: %s\n",__FILE__, __LINE__, mysql_error(conn));
		conn = NULL;
		return ;
	}
	else
...
QueryResult Connection::executeSelectQuery(string statement)
{
	...
	try
	{
		...
		while(true)
		{
			...
			for(unsigned int i=0; i < mysql_num_fields(resultSet); i++)
			{
	...
	{
		cout<<__FILE__<<":"<<__LINE__<<", executeSelectQuery faild"<<endl;
	}
	return queryResult;
}

int Connection::executeReplaceQuery(string statement)
{
	const char* query = statement.c_str();
